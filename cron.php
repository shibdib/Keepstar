            ...
            if ($id !== $characterData->corporation_id) {
                foreach ($roles as $role) {
                    if ($role->name === $authGroup["role"]) {
                        ...
                        if ((int)$config['discord']['logChannel'] !== 0) { $restcord->channel->createMessage(['channel.id' => (int)$config['discord']['logChannel'], 'content' => "$characterData->name has been removed from the role $role->name"]);}
